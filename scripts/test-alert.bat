@echo off
REM TC2-2: Quick test script for SendAlerts Alert Center
...
echo [test-alert] Sending alert to SendAlerts...
echo [test-alert] Group: %GROUP%
echo [test-alert] Message: %MESSAGE%

powershell -NoProfile -Command ^
    "$msg = '{\"GroupName\":\"%GROUP%\",\"CustomMessage\":\"%MESSAGE%\"}'; " ^
    "$pipe = New-Object System.IO.Pipes.NamedPipeClientStream('.', 'sendalerts-pipe', 'Out'); " ^
