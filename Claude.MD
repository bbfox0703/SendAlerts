...
## Build Commands
- **Build**: `dotnet build`
- **Run**: `dotnet run --project SendAlerts.Desktop`
- **Clean**: `dotnet clean`
...
┌─────────────────────────────────────────────────────────────────┐
│                    Interface Tier (接口層)                       │
│  Named Pipe Server: \\.\pipe\sendalerts-pipe                   │
...
│  Example: "Critical" → [Discord_Admin, Telegram_All, Email_IT]   │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
│  Example: Discord_User1, Discord_Admin1, Telegram_All, Email_IT │
└─────────────────────────────────────────────────────────────────┘
```

### Solution Structure
```
SendAlerts/           (Core - cross-platform logic, namespace: SendAlerts)
...
SendAlerts.Desktop/   (Platform head, namespace: SendAlerts.Desktop)
├── Implementations/  NvmlWindowsProvider, SingleInstanceManager
└── Program.cs        Single instance mutex + Named Pipe init
```

**Note**: Namespace is `SendAlerts` (previously was `_16pin_vmon` with underscores)
...
┌───────────────────────────────────────────┐
│ IAlertAction[] (Discord, Telegram, Email...) │
└───────────────────────────────────────────┘
```
...
; Arguments: -Command "'{\"GroupName\":\"Critical\",\"CustomMessage\":\"GPU Temp: <#GPU Core Temp#>°C\"}' | Out-File -FilePath \\.\pipe\sendalerts-pipe -Encoding utf8"
```
...
- **Platform Abstraction**: Any system/OS-dependent call (P/Invoke, Registry, OS commands) MUST go through an interface in the Core project (`SendAlerts/Interfaces/`), with implementations in platform-specific projects. The Core project (`SendAlerts/`) must NEVER contain direct platform-specific code.
...
- **Log messages**: All `Log.*()` messages MUST be in English only. No Chinese/Japanese/other non-ASCII text in log output. Use structured logging parameters for variable data.

## Known Constraints

### ✅ Avalonia 12.x (upgraded 2026-07-25 from 11.x)
The project ran on Avalonia 11.x until `ScottPlot.Avalonia 5.1.59` (released 2026-06-22)
added Avalonia 12 support. Earlier ScottPlot (`5.1.58`) referenced
`Avalonia.Platform.OptionalFeatureProviderExtensions` (removed in Avalonia 12), so any 12.x
bump threw `System.TypeLoadException` from `ScottPlot.Avalonia.AvaPlot.CustomDrawOp.Render`
(fully transparent/black window). `5.1.59` is a **breaking** release requiring Avalonia
>= 12.0.0 and is **not** backward-compatible with 11.x — **do NOT downgrade**.

**Current state**: `Directory.Packages.props` pins all Avalonia.* packages to
`[12.1.0, 13.0.0)` (range blocks the NuGet GUI from offering 13.x) and
`ScottPlot.Avalonia 5.1.59`.

**Migration that was applied** (reference for any similar future bump):
- `Avalonia.Diagnostics` was **removed in Avalonia 12** → replaced with
  `AvaloniaUI.DiagnosticsSupport 2.2.0`. DevTools now works via the standalone "Avalonia
  Developer Tools" app + `AttachDeveloperTools()`; this app never called `AttachDevTools()`,
  so there is no behavioural impact.
- XAML/code renames (old names kept `[Obsolete]` in 12 but updated anyway):
  `SystemDecorations` → `WindowDecorations` (`Views/*.axaml` + `TrayIconManager.cs`),
  `Watermark` → `PlaceholderText` (all dialogs),
  `NativeMenuItemToggleType` → `MenuItemToggleType` (tray menu).
- **Text shaping is no longer auto-registered** for manual backend config: added the
  `Avalonia.HarfBuzz` package + `.UseHarfBuzz()` to `BuildAvaloniaApp()` in **both**
  `SendAlerts.Desktop` and `SendAlerts.Watchdog`. Without it `AppBuilder.Setup()` throws
  `InvalidOperationException: No text shaping system configured` at startup — the process
  dies before any window/tray icon appears (builds fine; only fails at runtime).
- Compiled bindings are enabled by default in Avalonia 12; all existing XAML compiled clean.
- `Avalonia.Angle.Windows.Natives 2.1.27548.20260419` was already the version Avalonia.Win32
  12.1.0 wants — left unchanged.

**Still verify after any Avalonia bump**: smoke-test all 4 `AvaPlot` charts + the custom
`GradientFillPlottable` + crosshair/tooltip layer render correctly (this was the exact
failure mode of the old 11→12 attempt).

### ⚠️ Native AOT is NOT supported (ship JIT + single-file)
...
1. ~~ScottPlot.Avalonia pins Avalonia to 11.x~~ **CLEARED 2026-07-25**: upgraded to
   `ScottPlot.Avalonia 5.1.59` + Avalonia 12.1.0 (see Known Constraints above). ScottPlot
   is still reflection-heavy and its AOT-compatibility is unverified, but the Avalonia 11
   pin that blocked everything else is gone.
...
- `BuildAvaloniaApp()` uses `UsePlatformDetect()` (reflection backend load) and lists
  `Win32CompositionMode.WinUIComposition` first — AOT needs explicit
  `.UseWin32().UseSkia().UseHarfBuzz()` and `RedirectionSurface`-only composition.
- `Avalonia.Desktop` package is referenced (Desktop + Watchdog) — on win-x64 AOT this
  drags X11/FreeDesktop/DBus; reference Win32/Skia/HarfBuzz explicitly instead.

Reference: `D:\Github\CE-Handwire-Private\docs\aot-pitfalls.md` (the Avalonia 12 +
Native AOT landmine catalogue this gap list was checked against).
